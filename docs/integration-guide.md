...
The TypeScript SDK lives in this repository under `sdk/typescript`. The intended package name is `@blocksifr/ttp-sdk`. Until publication, treat this section as the stable integration shape and use the local demo for a runnable first pass.

```bash
# Once published:
npm install @blocksifr/ttp-sdk
```
...
```typescript
import { verifyTTPToken } from "@blocksifr/ttp-sdk"
